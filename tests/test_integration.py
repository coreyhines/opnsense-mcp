...
async def test_system_tool(tool):
    ...
    logger.info(f"System status: CPU: {status.get('cpu_usage')}%, Memory: {status.get('memory_usage')}%")
...
async def test_interface_tool(tool):
    ...
    if interfaces.get('interfaces'):
        ...
        logger.info(f"Interface {if_name} status: {details.get('interface', {}).get('status', 'unknown')}")
...
async def test_service_tool(tool):
    ...
    if services.get('services'):
        ...
        logger.info(f"Service status: {details.get('service', {}).get('status', 'unknown')}")
...
async def test_traffic_tool(tool):
    ...
    status = await tool.execute({"action": "status"})
    logger.info(f"Traffic shaper status: {'enabled' if status.get('enabled') else 'disabled'}")
...
async def main():
    ...
    parser.add_argument('--config', type=str, default=os.path.join(os.path.dirname(__file__), 'vars/key.yaml'),
                        help='Path to config file')
    parser.add_argument('--tool', type=str, choices=['system', 'arp', 'interface', 'firewall', 'service', 
                                                    'vpn', 'dns', 'traffic', 'ids', 'certificate'], 
