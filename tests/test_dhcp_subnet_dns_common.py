...
def test_merge_single_address_updates_slot_one() -> None:
    merged = merge_slot_update(
        ["172.20.10.5"],
        dns_server="172.20.10.4",
        family="ipv4",
    )
    assert merged == ["172.20.10.4"]


def test_merge_single_address_updates_slot_two() -> None:
    merged = merge_slot_update(
        ["172.20.10.4"],
        dns_server="172.20.10.5",
        slot=2,
        family="ipv4",
    )
    assert merged == ["172.20.10.4", "172.20.10.5"]


def test_merge_two_addresses_replace_both_slots() -> None:
    merged = merge_slot_update(
        ["172.20.10.1"],
        dns_servers=["172.20.10.4", "172.20.10.5"],
        family="ipv4",
    )
    assert merged == ["172.20.10.4", "172.20.10.5"]
...
def test_merge_rejects_both_single_and_list() -> None:
    with pytest.raises(ValueError, match="not both"):
        merge_slot_update(
            [],
            dns_server="172.20.10.4",
            dns_servers=["172.20.10.5"],
            family="ipv4",
        )


def test_parse_and_format_ipv6_with_brackets() -> None:
    parsed = parse_dns_server_list("[2001:db8:5eed:b501::44]", "ipv6")
    assert parsed == ["2001:db8:5eed:b501::44"]
    formatted = format_dns_server_list(parsed, "ipv6")
    assert formatted == "[2001:db8:5eed:b501::44]"


def test_validate_address_family_mismatch() -> None:
    with pytest.raises(ValueError, match="Expected IPv4"):
        validate_address("2001:db8:5eed:b501::44", "ipv4")
