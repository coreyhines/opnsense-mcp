...
class APITester:
    ...
    async def setup(self):
        ...
        try:
            ...
            logger.info(
                f"Connected to OPNsense at {self.config['opnsense']['firewall_host']}"
            )
...
async def main():
    ...
    parser.add_argument(
        "--config",
        type=str,
        default="./examples/mcp.json",
        help="Path to configuration file (JSON or YAML)",
    )
    parser.add_argument("--tool", type=str, help="Specific tool to test")
    parser.add_argument(
        "--action", type=str, help="Action to perform (requires --tool)"
    )
    parser.add_argument(
        "--params",
        type=str,
        help="Parameters as JSON string (requires --tool and --action)",
    )
