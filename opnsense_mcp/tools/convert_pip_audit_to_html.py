...
def main():
    ...
    if not data or isinstance(data, str):
        ...
        html.append("<table border='1'><tr><th>Package</th><th>Version</th><th>Vuln ID</th><th>Description</th><th>Fix Version</th></tr>")
        for entry in data:
            pkg = entry.get("name", "")
            ...
            for vuln in entry.get("vulns", []):
                ...
                html.append(f"<tr><td>{pkg}</td><td>{version}</td><td>{vuln_id}</td><td>{desc}</td><td>{fix}</td></tr>")
        html.append("</table>")
