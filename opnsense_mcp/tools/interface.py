    ...
    async def execute(self, params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if action not in ["list", "get"]:
                raise ValueError(f"Invalid action: {action}. Supported actions are 'list' and 'get'")
                ...
                try:
                    ...
                    for intf in interfaces:
                        intf_data = {
                            "name": intf,
                            "status": "active",
                            "ipv4_neighbors": [e for e in arp_data if e.get('intf') == intf],
                            "ipv6_neighbors": [e for e in ndp_data if e.get('intf') == intf]
                        }
            ...
            # Handle get action
                ...
                try:
                    ...
                    ipv4_entries = [e for e in arp_data if e.get('intf') == interface_name]
                    ipv6_entries = [e for e in ndp_data if e.get('intf') == interface_name]
                    ...
                    if ipv4_entries or ipv6_entries:
                        ...
                        raise ValueError(f"Interface {interface_name} not found or has no active neighbors")
