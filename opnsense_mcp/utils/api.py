...
class OPNsenseClient:
        ...
        try:
            ...
            if response.status_code == 200:
                try:
                    ...
                    if isinstance(json_data, dict) and json_data.get('result') == 'failed':
    ...
    async def get_system_status(self):
        ...
        try:
            ...
            try:
                sys_info = await self._make_request('GET', ENDPOINTS['system']['information'])
                if isinstance(sys_info, dict):
                    # Extract temperature data if available
                    if 'temperature' in sys_info:
                        for sensor in sys_info.get('temperature', []):
                            if isinstance(sensor, dict) and 'device' in sensor and 'temperature' in sensor:
                                status_data['temperature'][sensor['device']] = sensor['temperature']
                    
                    # Extract any additional system information
                    if 'product' in sys_info:
                        status_data['versions']['product'] = sys_info['product']
            except Exception as e:
                logger.warning(f"Could not fetch additional system information: {str(e)}")
    ...
    async def add_firewall_rule(self, rule_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            response = await self._make_request('POST', ENDPOINTS['firewall']['add_rule'], json={"rule": rule_data})
    ...
    async def update_firewall_rule(self, uuid: str, rule_data: Dict[str, Any]) -> Dict[str, Any]:
        """Update an existing firewall rule"""
        try:
            logger.debug(f"Updating firewall rule {uuid} with data: {rule_data}")
            response = await self._make_request('POST', f"{ENDPOINTS['firewall']['set_rule']}/{uuid}", json={"rule": rule_data})
            
            if response.get('result') != 'saved':
                raise ResponseError(f"Failed to update firewall rule: {response.get('message', 'Unknown error')}")
    ...
    async def delete_firewall_rule(self, uuid: str) -> Dict[str, Any]:
        ...
        try:
            ...
            response = await self._make_request('POST', f"{ENDPOINTS['firewall']['del_rule']}/{uuid}")
            
            if not isinstance(response, dict) or response.get('result') != 'deleted':
                raise ResponseError(f"Failed to delete firewall rule: {response.get('message', 'Unknown error')}")
    ...
    async def toggle_firewall_rule(self, uuid: str, enabled: bool) -> Dict[str, Any]:
        ...
        try:
            ...
            response = await self._make_request('POST', f"{ENDPOINTS['firewall']['toggle_rule']}/{uuid}/{status}")
            
            if not isinstance(response, dict) or response.get('result') != 'ok':
                raise ResponseError(f"Failed to toggle firewall rule: {response.get('message', 'Unknown error')}")
                
            logger.info(f"Successfully {'enabled' if enabled else 'disabled'} firewall rule {uuid}")
    ...
    async def apply_firewall_changes(self) -> Dict[str, Any]:
        ...
        try:
            ...
            savepoint_resp = await self._make_request('POST', '/api/firewall/filter/savepoint')
            ...
            apply_resp = await self._make_request('POST', f'/api/firewall/filter/apply/{revision}')
            
            if apply_resp.get('status') != 'ok':
                raise ResponseError(f"Failed to apply firewall changes: {apply_resp.get('message', 'Unknown error')}")
    ...
    async def cancel_firewall_rollback(self, revision: str) -> Dict[str, Any]:
        ...
        try:
            ...
            response = await self._make_request('POST', f'/api/firewall/filter/cancel_rollback/{revision}')
            
            if response.get('status') != 'ok':
                raise ResponseError(f"Failed to cancel rollback: {response.get('message', 'Unknown error')}")
