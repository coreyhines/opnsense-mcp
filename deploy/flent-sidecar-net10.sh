...
podman run --rm \
  ...
  "${IMAGE}" bash -ec "
    ...
    flent -i /out/$(basename "${OUT_FLENT}").gz -f summary 2>/dev/null \
      | tee /out/$(basename "${OUT_TXT}" .txt)_summary.txt || \
    flent -i /out/$(basename "${OUT_FLENT}") -f summary 2>/dev/null \
      | tee /out/$(basename "${OUT_TXT}" .txt)_summary.txt || true
  "
