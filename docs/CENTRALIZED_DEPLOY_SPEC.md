...
- **Images:** **Forgejo Actions** pushes **`hub.frobozz.example/opnsense-mcp:<semver>`** on git tag **`vX.Y.Z`**, or **`X.Y.Z-dev.<short-sha>`** on `main` (version from `pyproject.toml`). Host install **pulls** a pinned tag (no `:latest`).
...
| Clients | **`https://opnsense-mcp.frobozz.example/sse`** (after TLS; DNS A/AAAA to pod IP — see **`deploy/TLS.md`**). |
...
- **Default clone URL (Forgejo):** `https://forgejo.frobozz.example/coreyhines/opnsense-mcp.git` — override with **`OPNSENSE_MCP_REPO_URL`**.
...
- **Image (Podman):** **`OPNSENSE_MCP_IMAGE_REPO`** (default **`hub.frobozz.example/opnsense-mcp`**) and **`OPNSENSE_MCP_IMAGE_TAG`** (**required**, semver **`X.Y.Z`** or **`X.Y.Z-dev.<sha>`**; not **`latest`**). Omit **`OPNSENSE_MCP_IMAGE_TAG`** to auto-select via **`deploy/ci/compute-image-tag.sh`**.
- **Caddy image:** **`OPNSENSE_MCP_CADDY_IMAGE`** (default **`docker.io/library/caddy:2.9.1-alpine`**).
- **Podman (default):** clones/updates repo for deploy assets, **pulls** pinned image, installs quadlets under **`/etc/containers/systemd/opnsense-mcp/`**, creates **`$INSTALL_ROOT/environment`** and **`$INSTALL_ROOT/Caddyfile`** if missing, then **`systemctl enable`** / **`start`**.
...
- **One-liner (raw script on `main`):**
  ```bash
  curl -fsSL 'https://forgejo.frobozz.example/coreyhines/opnsense-mcp/raw/branch/main/deploy/install.sh' | sudo bash
  ```
...
| `OPNSENSE_MCP_IMAGE_REPO` | Registry image (default `hub.frobozz.example/opnsense-mcp`). |
...
sudo env OPNSENSE_MCP_IMAGE_TAG=1.0.0 \
  OPNSENSE_MCP_NETWORK=net-10 OPNSENSE_MCP_IP=172.20.10.50 \
...
- **Unit not found after install:** (1) Quadlet searches `/etc/containers/systemd/` recursively, so a per-service subdirectory works and is the layout used here. Only Podman **before 4.7** ignored subdirectories ([containers/podman#20236](https://github.com/containers/podman/issues/20236)); verify with `podman --version` before assuming a flat layout is required. Confirm what the generator actually sees with the dry-run in (4) rather than inferring it. (2) Quadlet names `.pod` units as **`<stem>-pod.service`** (appends `-pod`), **not** `pod-<stem>.service`: **`opnsense-mcp.pod`** → **`opnsense-mcp-pod.service`**. Container **`Requires=`** must match. (3) **`systemctl daemon-reload`**; ensure **`podman-quadlet`** is installed. (4) Dry-run: `sudo /usr/lib/systemd/system-generators/podman-system-generator --dryrun 2>&1 | grep opnsense-mcp`.
